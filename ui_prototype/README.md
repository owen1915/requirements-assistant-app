...
*(component (a). See the [repository README](../README.md) for the other three.)*
...
2. Create a config file called `ui_prototype/backend/.env`
3. Stop and ask you to add your API key to that file

**Add your API key:**

The file `ui_prototype/backend/.env` will appear in the left file panel in VSCode under `ui_prototype/backend`. Click it to open it. It looks like this:
...
Click Anthropic (Claude) or OpenAI (GPT-4o). If your key is already in `ui_prototype/backend/.env` you will see a green confirmation and do not need to enter it again.
...
The backend server did not start. Look at the VSCode terminal for error messages. Most common cause: `ui_prototype/backend/.env` is missing or has the wrong key.

**"No API key provided" error**
Open `ui_prototype/backend/.env` in VSCode and confirm your key is filled in and `AI_PROVIDER` matches the key you added (`anthropic` or `openai`).
